...
bool test_less_than(){
    ...
    result = test_less_than_uint8();
    tests++;
    if(!result){
        std::cout << "failed test_less_than_uint8\n";
        fails++;
    }
    
    result = test_less_than_int8();
    tests++;
    if(!result){
        std::cout << "failed test_less_than_int8\n";
        fails++;
    }
    
    result = test_less_than_uint16();
    tests++;
    if(!result){
        std::cout << "failed test_less_than_uint16\n";
        fails++;
    }
    
    result = test_less_than_int16();
    tests++;
    if(!result){
        std::cout << "failed test_less_than_int16\n";
        fails++;
    }
    
    result = test_less_than_uint32();
    tests++;
    if(!result){
        std::cout << "failed test_less_than_uint32\n";
        fails++;
    }
    
    result = test_less_than_int32();
    tests++;
    if(!result){
        std::cout << "failed test_less_than_int32\n";
        fails++;
    }
    ...
    tests++;
    if(!result){
        std::cout << "failed test_less_than_int64\n";
        fails++;
    }
    
    result = test_less_than_float();
    tests++;
    if(!result){
        std::cout << "failed test_less_than_float\n";
        fails++;
    }
    
    result = test_less_than_double();
    tests++;
    if(!result){
        std::cout << "failed test_less_than_double\n";
        fails++;
    }
    
    
    std::cout << "test less-than done " << tests << " tests, " << fails << " failures\n";
    return fails == 0;
}
