#include <iostream>
#include "test_not_equal.hpp"



bool test_not_equal_uint8(){
...
bool test_not_equal(){
    ...
    result = test_not_equal_uint8();
    tests++;
    if(!result){
        std::cout << "failed test_not_equal_uint8\n";
        fails++;
    }
    
    result = test_not_equal_int8();
    tests++;
    if(!result){
        std::cout << "failed test_not_equal_int8\n";
        fails++;
    }
    
    result = test_not_equal_uint16();
    tests++;
    if(!result){
        std::cout << "failed test_not_equal_uint16\n";
        fails++;
    }
    
    result = test_not_equal_int16();
    tests++;
    if(!result){
        std::cout << "failed test_not_equal_int16\n";
        fails++;
    }
    
    result = test_not_equal_uint32();
    tests++;
    if(!result){
        std::cout << "failed test_not_equal_uint32\n";
        fails++;
    }
    
    result = test_not_equal_int32();
    tests++;
    if(!result){
        std::cout << "failed test_not_equal_int32\n";
        fails++;
    }
    
    result = test_not_equal_uint64();
    tests++;
    if(!result){
        std::cout << "failed test_not_equal_uint64\n";
        fails++;
    }
    
    result = test_not_equal_int64();
    tests++;
    if(!result){
        std::cout << "failed test_not_equal_int64\n";
        fails++;
    }
    
    result = test_not_equal_float();
    tests++;
    if(!result){
        std::cout << "failed test_not_equal_float\n";
        fails++;
    }
    
    result = test_not_equal_double();
    tests++;
    if(!result){
        std::cout << "failed test_not_equal_double\n";
        fails++;
    }
    
    
    std::cout << "test not-equal done " << tests << " tests, " << fails << " failures\n";
    return fails == 0;
}
